...
(function ($, window, document, undefined) {
  ...
  function paginga(element, options) {
    ...
    this.items = $(this.element).find(
      this.settings.itemsContainer + " " + this.settings.item
    );
  ...
  $.extend(paginga.prototype, {
    init: function () {
      ...
      if (this.settings.history) {
        ...
        if (window.location.hash) {
          var hash = parseInt(
            window.location.hash.substring(
              plugin.settings.historyHashPrefix.length + 1
            ),
            10
          );
        ...
        window.addEventListener("popstate", function (event) {
          plugin.currentPage =
            event && event.state && event.state.page
              ? event.state.page
    ...
    showPage: function () {
      ...
      if (
        plugin._ready &&
        plugin.settings.scrollToTop &&
        element.offset().top - plugin.settings.scrollToTop.offset <
          $(window).scrollTop()
      ) {
        $("html, body").animate(
          {
        ...
        if (this.settings.maxPageNumbers) {
          ...
          if (lastPage - firstPage < this.settings.maxPageNumbers - 1) {
            if (firstPage <= offset) {
              lastPage = Math.min(
                this.totalPages,
                firstPage + this.settings.maxPageNumbers - 1
              );
            } else if (lastPage > this.totalPages - offset) {
              firstPage = Math.max(
                1,
                lastPage - this.settings.maxPageNumbers + 1
              );
            }
          }
        }
        ...
        for (var pageNumber = firstPage; pageNumber <= lastPage; pageNumber++) {
          var className =
            pageNumber == this.currentPage
              ? this.settings.currentPageClass
              : "";
    ...
    setHistoryUrl: function () {
      ...
      if (plugin._ready && plugin.settings.history && "pushState" in history) {
        history.pushState(
          ...
          null,
          "#" + plugin.settings.historyHashPrefix + this.currentPage
        );
      }
    },
  });
